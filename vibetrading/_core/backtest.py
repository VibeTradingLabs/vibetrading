...
class BacktestEngine:
    ...
    def __init__(
        ...
        self._initial_total_value: float | None = None
        self._all_time_peak: float | None = None
        self._max_drawdown_observed: float | None = None
    ...
    def _should_stream_12hourly(self) -> bool:
        if not hasattr(self, "_last_stream_12hour"):
            if self.sandbox.current_time:
                ...
                self._last_stream_12hour = self.sandbox.current_time.replace(
    ...
    def _record_results(self, timestamp: datetime):
        ...
        self.results.append(
            {
                "timestamp": timestamp,
                "balances": self.sandbox.balances.copy(),
        ...
        if ENABLE_STRUCTURED_LOGGING and self._should_stream_metrics():
            ...
            log_metrics_update(
                ...
                funding_revenue=m.get("funding_revenue", 0.0),
                total_tx_fees=m.get("total_tx_fees", 0.0),
                initial_balance=ib,
    ...
    # ── Main run ───────────────────────────────────────────────────────
    def run(self, strategy_code: str | None = None) -> dict[str, Any] | None:
        """
